...
internal struct Bit32Library: LuaLibrary {
    ...
    public let bnot = LuaSwiftFunction {state, args in
        return [.number(Double(~UInt32(bitPattern: Int32(truncatingIfNeeded: try args.checkInt(at: 1)))))]
    }

    public let lshift = LuaSwiftFunction {state, args in
        return [.number(Double(UInt32(bitPattern: Int32(truncatingIfNeeded: try args.checkInt(at: 1))) << (try args.checkInt(at: 2))))]
    }
    ...
    public let lrotate = LuaSwiftFunction {state, args in
        let input = UInt32(bitPattern: Int32(truncatingIfNeeded: try args.checkInt(at: 1)))
        let shift = try args.checkInt(at: 2) & 0x1F
    ...
    public let extract = LuaSwiftFunction {state, args in
        let input = UInt32(bitPattern: Int32(truncatingIfNeeded: try args.checkInt(at: 1)))
        let field = try args.checkInt(at: 2)
        let width = try args.checkInt(at: 3, default: 1)
        if field < 0 || field + width > 32 {
            throw Lua.error(in: state, message: "trying to access non-existent bits")
        }
    ...
    public let replace = LuaSwiftFunction {state, args in
        let input = UInt32(bitPattern: Int32(truncatingIfNeeded: try args.checkInt(at: 1)))
        let value = UInt32(bitPattern: Int32(truncatingIfNeeded: try args.checkInt(at: 2)))
        let field = try args.checkInt(at: 3)
        let width = try args.checkInt(at: 4, default: 1)
        if field < 0 || field + width > 32 {
            throw Lua.error(in: state, message: "trying to access non-existent bits")
        }
